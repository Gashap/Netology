...
import ru.netology.service.AsyncInputOperationService;
import ru.netology.service.StatementService;
...
public class OperationController {
	...
	public ResponseEntity<Iterable<Operation>> getClientOperations(@PathVariable Integer customerId) {
		...
		if (operations != null) return new ResponseEntity<>(operations, HttpStatus.OK);
	...
	public ResponseEntity<Operation> deleteOperation(@PathVariable int id) {
		...
		if (deletedOperation.get() != null) return new ResponseEntity<>(deletedOperation.get(), HttpStatus.OK);
		return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	}
}
